...
import { pool, actionIds, placeOrder, fakeIp, BID } from "./lib.mjs";
const n = Number(process.argv[2] ?? 60);
const ids = actionIds(); const db = pool();
const { rows } = await db.query(`select id from "MenuItem" where "businessId" = ${BID} and "isAvailable" and "deletedAt" is null and "trackInventory" = false order by id`);
...
const r = await db.query(`select status, count(*)::int n from "Order" where "businessId" = ${BID} group by status order by status`);
